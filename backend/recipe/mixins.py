...
from .models import Recipe, RecipeIngredient, RecipeTag, Favorite, Basket
...
class RecipeValidationMixin:
    ...
    def validate_tags(self, value):
        ...
        duplicate_tags = [tag_id for tag_id, count in tag_counts.items() if count > 1]

        if duplicate_tags:
            raise serializers.ValidationError({
                "tags": f"Обнаружены повторяющиеся id тегов: {duplicate_tags}"
            })
    ...
    def validate_ingredients(self, value):
        ...
        duplicate_ingredients = [ingredient_id for ingredient_id, count in ingredient_counts.items() if count > 1]

        if duplicate_ingredients:
            raise serializers.ValidationError({
                "ingredients": f"Обнаружены повторяющиеся id ингредиентов: {duplicate_ingredients}"
            })
...
class Favorit_ShoppingCart_Save_MethodsMixin:
    def get_is_favorited(self, obj):
        return not self.context.get('request').user.is_anonymous and Favorite.objects.filter(user=self.context['request'].user, recipe=obj).exists()
    
    def get_is_in_shopping_cart(self, obj):
        return not self.context.get('request').user.is_anonymous and Basket.objects.filter(user=self.context['request'].user, recipe=obj).exists()
...
# Вынос общих объектов, используемых в проекте, имеющих изначальную зависимость с apps recipe.
