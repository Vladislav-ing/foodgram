...
class AvatarProfileSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        """Если у пользователя уже есть аватар и он обновляет его, удаляем старый файл"""
...
class FullProfileSerializer(CreateProfileSerializer):
    ...
    def get_is_subscribed(self, obj):
        ...
        if current_user.is_authenticated and current_user.subscription.all().filter(id=obj.id):
            return True
...
class ResetPasswordUser(serializers.Serializer):
    ...
    def validate_current_password(self, value):
        ...
        if not user.check_password(value):
            raise serializers.ValidationError('Действующий пароль не правильный.')
        return value
    ...
    def validate(self, attrs):
        ...
        if current_password == new_password:
            raise serializers.ValidationError('Новый пароль, не может быть равен старому.')
...
class SubscriptionProfileSerializer(FullProfileSerializer):
    """
    Серилизатор для обработки запросов к subscribe, возвращает полные данные пользователя,
    а также 2 доп. поля recipes, recipes_count. Данные пользователя обрабатываются только в
    режиме чтения.
    """
    ...
    class Meta(FullProfileSerializer.Meta):
        fields = FullProfileSerializer.Meta.fields + ('recipes', 'recipes_count')
        read_only_fields = (
            'email', 'id', 'username', 'first_name',
            'last_name', 'is_subscribed', 'avatar',
